...
function WPPortfolio_refresh_forceThumbnailRefresh($siteid)
{
	...
	else 
	{
		...
		$newImageURL = WPPortfolio_getThumbnail($websitedetails['siteurl'], 'sm', ($websitedetails['specificpage'] == 1), true);
	}
...
{	
	...
	if ($verbose) 
	{
		...
		WPPortfolio_thumbnails_status(sprintf(__('You have <b>%d items in your portfolio</b>, of which <b>%d are websites that use STW</b> for thumbnails.', 'wp-portfolio'), $totalCount, $totalCountSTW), 0, 1);
	}
